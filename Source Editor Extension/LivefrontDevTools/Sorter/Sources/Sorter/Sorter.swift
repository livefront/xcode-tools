...
public enum Sorter {
    ...
    ///
    /// - Parameter lines: The lines of code to be sorted.
    ...
    public static func sort(_ lines: [String], isNestedType: Bool = false) -> [String] {
        ...
        let recursivelySortedSections = sectionsWithSortedBlocks.map { section in
            ...
            return Section(blocks: recursivelySortedBlocks, footer: section.footer, header: section.header)
        }
    ...
    /// Divide the lines of code into MARK sections at the same level, ignoring MARKs nested within classes
    /// or structs until they're recursively sorted.
    private static func getMarkSections(from lines: [String]) -> [[String]] {
        ...
        // Ignore any MARKs at index 0 and add an index at the end of the array to ensure proper division into sections.
        if markIndices.contains(0) { markIndices.remove(at: 0) }
        markIndices.append(lines.count)

        // Split the code into sections around each MARK divider, including the MARK at the top of the section.
        var prevIndex = 0
        var sections = [[String]]()
    ...
    /// Converted sorted sections back into raw lines of code.
    ...
    /// Divide code in sections as denoted by `MARK` dividers.
        ...
        for (index, line) in lines.enumerated() {
            ...
            if index == 0, line.contains("MARK") {
                ...
                if currentBlock.type == nil, let type = line.blockType {
                    ...
                    // If the current line is within parentheses or brackets, or is a closing bracket,
                    ...
                    // Skip over blank lines that aren't in the body of the block to avoid formatting errors.
                } else {
                    // Otherwise, if the line isn't part of the body, isn't a comment, and doesn't contain an keyword
                    // or macro that would indicate it's part of the next block, add it to the previous block (this
    ...
    /// Sort the blocks within each section, first by type and then alphabetically.
